...
// Process: s s~ > zp > mu- mu+ mu- mu+ WEIGHTED=8

//--------------------------------------------------------------------------
// Initialize process.

void Spin1_DN_SF::initProc(string param_card_name) 
{
...
void Spin1_DN_SF::updateProc(SLHAReader slha) 
{
  pars->setIndependentParameters(slha); 
  pars->setIndependentCouplings(); 
  ...
  if (sum_hel == 0 || ntry < 10)
  {
    ...
    for(int ihel = 0; ihel < ncomb; ihel++ )
    {
      if (goodhel[ihel] || ntry < 2)
      {
        ...
        t[0] = matrix_ssx_zp_mummupmummup(); 
        ...
        t[1] = matrix_ssx_zp_mummupmummup(); 
        double tsum = 0; 
...
void Spin1_DN_SF::calculate_wavefunctions(const int perm[], const int hel[])
{
