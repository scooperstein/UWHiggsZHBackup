...
// Process: s s~ > xg > mu- mu+ mu- mu+ WEIGHTED=8

//--------------------------------------------------------------------------
// Initialize process.

void Spin2_qq_DN_SF::initProc(string param_card_name) 
{
...
void Spin2_qq_DN_SF::updateProc(SLHAReader slha) 
{
  pars->setIndependentParameters(slha); 
  pars->setIndependentCouplings(); 
  ...
  if (sum_hel == 0 || ntry < 10)
  {
    ...
    for(int ihel = 0; ihel < ncomb; ihel++ )
    {
      if (goodhel[ihel] || ntry < 2)
      {
        ...
        t[0] = matrix_ssx_xg_mummupmummup(); 
        ...
        t[1] = matrix_ssx_xg_mummupmummup(); 
        double tsum = 0; 
...
double Spin2_qq_DN_SF::sigmaHat() 
{
...
void Spin2_qq_DN_SF::calculate_wavefunctions(const int perm[], const int hel[])
{
...
//   int i, j; 
  ...
  FFT1_2_3_4_3(w[0], w[1], pars->HEF_MEKD_GC_135, pars->HEF_MEKD_GC_132, pars->HEF_MEKD_GC_133,
      pars->HEF_MEKD_GC_134, pars->MXG, pars->WXG, w[6]);
...
double Spin2_qq_DN_SF::matrix_ssx_xg_mummupmummup() 
{
  int i, j; 
  // Local variables
//   const int ngraphs = 2; 
