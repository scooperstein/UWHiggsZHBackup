...
// Process: c c~ > zp > mu- mu+ mu- mu+ WEIGHTED=8

//--------------------------------------------------------------------------
// Initialize process.

void Spin1_UP_SF::initProc(string param_card_name) 
{
...
void Spin1_UP_SF::updateProc(SLHAReader slha) 
{
  pars->setIndependentParameters(slha); 
  pars->setIndependentCouplings(); 
  ...
  if (sum_hel == 0 || ntry < 10)
  {
    ...
    for(int ihel = 0; ihel < ncomb; ihel++ )
    {
      if (goodhel[ihel] || ntry < 2)
      {
        ...
        t[0] = matrix_ccx_zp_mummupmummup(); 
        ...
        t[1] = matrix_ccx_zp_mummupmummup(); 
        double tsum = 0; 
...
void Spin1_UP_SF::calculate_wavefunctions(const int perm[], const int hel[])
{
...
//   int i, j; 
  ...
  FFV1_2_3_4_3(w[0], w[1], pars->HEF_MEKD_GC_111, pars->HEF_MEKD_GC_108, pars->HEF_MEKD_GC_109,
      pars->HEF_MEKD_GC_110, pars->MZp, pars->WZp, w[6]);
...
double Spin1_UP_SF::matrix_ccx_zp_mummupmummup() 
{
  int i, j; 
  // Local variables
//   const int ngraphs = 2; 
