...
void Spin2_gg_OF::initProc(string param_card_name) 
{
...
void Spin2_gg_OF::updateProc(SLHAReader slha) 
{
  pars->setIndependentParameters(slha); 
  pars->setIndependentCouplings(); 
  ...
  if (sum_hel == 0 || ntry < 10)
  {
    ...
    for(int ihel = 0; ihel < ncomb; ihel++ )
    {
      if (goodhel[ihel] || ntry < 2)
      {
        ...
        t[0] = matrix_gg_xg_emepmummup_no_g(); 
...
void Spin2_gg_OF::calculate_wavefunctions(const int perm[], const int hel[])
{
...
double Spin2_gg_OF::matrix_gg_xg_emepmummup_no_g() 
{
  int i, j; 
  // Local variables
//   const int ngraphs = 2; 
