...
// Process: s s~ > zp > e- e+ mu- mu+ WEIGHTED=8

//--------------------------------------------------------------------------
// Initialize process.

void Spin1_DN_OF::initProc(string param_card_name) 
{
...
void Spin1_DN_OF::updateProc(SLHAReader slha) 
{
  pars->setIndependentParameters(slha); 
  pars->setIndependentCouplings(); 
...
void Spin1_DN_OF::calculate_wavefunctions(const int perm[], const int hel[])
{
...
double Spin1_DN_OF::matrix_ssx_zp_emepmummup() 
{
  int i, j; 
  // Local variables
//   const int ngraphs = 1; 
